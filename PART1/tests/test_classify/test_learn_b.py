...
learn_w = 0.01
learn_b = learn_w * np.array([1.0, 0.67, 0.5, 0.33, 0.25])
dir = 'record/test_learn_b/'

if __name__ == '__main__':
    train_set, dev_set = util.k_mixture(path='../../../DATASET/train', total_class=10, dev_count=1)
    train_loop = 10
    hit_rate = np.zeros(len(learn_b))
    ...
    for i in range(len(learn_b)):
        alpha = learn_b[i]
        bp = BPN.BPNetwork(layers_list,
                           learn_w=learn_w, learn_b=alpha,
        ...
        dataFrame.to_csv(os.path.join(dir, str(alpha), str(alpha) + ".csv"), sep=",", index=False)
        pass

        if len(dev_set) != 0:
            hit_rate[i] = 1.0 * score / len(dev_set)

        ax1.plot(range(train_loop), loss_arr, '-', label='alpha=' + str(alpha) + ', hit rate=' + str(hit_rate[i]))

    for k in range(len(learn_b)):
        print("learn rate = ", learn_b[k], "hit rate = ", hit_rate[k])
