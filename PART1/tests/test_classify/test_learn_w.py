...
if __name__ == '__main__':
        ...
        try:
            with tqdm(total=len(train_set) * train_loop) as pbar:
                for num in range(train_loop):
                    bp.set_learn_rate(pow(0.8, num / 2) * alpha,
                                      pow(0.8, num / 2) * 0.005)
    ...
    for k in range(len(learn_w)):
        print("learn rate = ", learn_w[k], "hit rate = ", hit_rate[k])
