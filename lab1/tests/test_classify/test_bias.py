...
'''
模型存储位置：record/test_classify/test_bias/...
'''
...
if __name__ == '__main__':
    ...
    for i in range(len(bias)):
        ...
        bp.save(os.path.join(dir, str(b)))
        bp.load(os.path.join(dir, str(b)))
        ...
        dataFrame.to_csv(os.path.join(dir, str(b), str(b) + ".csv"), sep=",", index=False)
        pass

        if len(dev_set) != 0:
            hit_rate[i] = 1.0 * score / len(dev_set)

        ax1.plot(range(train_loop), loss_arr, '-', label='bias=' + str(b) + ', hit rate=' + str(hit_rate[i]))

    for k in range(len(bias)):
        print("bias = ", bias[k], "hit rate = ", hit_rate[k])
